...
def main() -> int:
    args = parse_request_args("Voice STT wrapper for tss-stack.")
    ...
    try:
        with httpx.Client(timeout=timeout_seconds) as client:
            submit = client.post(f"{base_url}/v1/stt", json=request_payload["body"], headers=headers)
            if submit.status_code == 404:
                submit = client.post(f"{base_url}/stt/jobs", json=request_payload["body"], headers=headers)
            submit.raise_for_status()
            accepted = submit.json()
            job_id = accepted["id"]
            status_url = request_payload.get("status_url") or f"{base_url}/stt/jobs/{job_id}"
        ...
        write_json(
            args.result_path,
            {
                ...
                "metadata": {"wrapper": "voice_stt_wrapper", "base_url": base_url},
            },
        )
        return 1
    ...
    write_json(
        args.result_path,
        {
            ...
            "language_detected": final_payload.get("language_detected"),
